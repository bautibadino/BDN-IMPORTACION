  ...
  try {
    ...
    if (existingProduct) {
      ...
      const productToUpdate: Partial<Omit<Product, "id" | "productLeadId">> = {
        ...
        internalCode: formData.internalCode || existingProduct.internalCode || null,
      }
...
export async function createOrUpdateProductFromLeadWithCategoriesAction(
  ...
  try {
    // Crear o actualizar el producto usando la función existente
    const result = await createOrUpdateProductFromLeadAction(formData)
    
    if (!result.success || !result.product) {
      return result
    }

    // Si se proporcionaron categorías, asignarlas al producto
    if (formData.categoryIds && formData.categoryIds.length > 0) {
      const { assignCategoriesToProduct } = await import("@/lib/categories")
      
      await assignCategoriesToProduct({
    ...
    return {
      ...result,
      message: result.message + (formData.categoryIds?.length ? ` Categorías asignadas: ${formData.categoryIds.length}` : '')
    }
  } catch (error) {
    console.error("Error in createOrUpdateProductFromLeadWithCategoriesAction:", error)
    return { success: false, message: "Error al procesar el producto con categorías." }
  }
}
